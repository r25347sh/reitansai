(function () {
  ...
  function friendlyErr(text, statusCode) {
    if (statusCode === 403 || (text && text.indexOf('Resource not accessible') >= 0)) {
      return '403: Tokenに書き込み権限がありません。GitHub → Settings → Developer settings → Fine-grained tokens で、reitansai と reitansai_backup の両方に Contents: Read and write を付与し、新しいTokenを admin.js に設定してください。';
    }
  ...
  function injectChrome(html, pagePath) {
    ...
    function absUrl(rel) {
      ...
      if (/^(https?:|data:|blob:|\/\/|mailto:|tel:|#)/i.test(s)) return s;
      try { return new URL(s, base).href; } catch (e) { return s; }
    }

    function absolutize(htmlStr) {
      return String(htmlStr)
        .replace(/\b(href|src|poster)=(["'])([^"']*)\2/gi, function (m, attr, q, url) {
          return attr + '=' + q + absUrl(url) + q;
        });
    }

    /* MENU.js のみ除去。MENU.css は残してレイアウトを守る */
    var cleaned = html
      .replace(/<script[^>]*MENU\/MENU\.js[^>]*>[\s\S]*?<\/script>/gi, '')
      .replace(/<script[^>]*src=["'][^"']*MENU\/MENU\.js["'][^>]*><\/script>/gi, '');

    cleaned = absolutize(cleaned);
    ...
    var style = [
      ...
      '.radial-menu-wrapper,.menu-fab,.header-auth,.menu-hint{display:none!important}',
  ...
  var ALLOWED_TEXT_TAGS = {
    P:1, H1:1, H2:1, H3:1, H4:1, H5:1, H6:1,
    SPAN:1, A:1, LI:1, LABEL:1, TD:1, TH:1,
    BLOCKQUOTE:1, FIGCAPTION:1, STRONG:1, EM:1, B:1, I:1, U:1, S:1, SMALL:1,
    CODE:1, PRE:1, MARK:1, TIME:1, ABBR:1, CITE:1, Q:1, DFN:1, SUB:1, SUP:1,
    DT:1, DD:1, SUMMARY:1
  };
  var FORBIDDEN_TAGS = {
    HTML:1, HEAD:1, BODY:1, SCRIPT:1, STYLE:1, LINK:1, META:1, TITLE:1,
    NOSCRIPT:1, TEMPLATE:1, SVG:1, IFRAME:1, OBJECT:1, EMBED:1, CANVAS:1,
    INPUT:1, SELECT:1, TEXTAREA:1, OPTION:1, BUTTON:1, FORM:1,
    VIDEO:1, AUDIO:1, SOURCE:1, TRACK:1, MAP:1, AREA:1
  };
  var STRUCTURAL_TAGS = {
    DIV:1, SECTION:1, ARTICLE:1, MAIN:1, ASIDE:1, HEADER:1, FOOTER:1, NAV:1,
    UL:1, OL:1, TABLE:1, THEAD:1, TBODY:1, TFOOT:1, TR:1, FIGURE:1,
    DETAILS:1, FIELDSET:1, LEGEND:1
  };
  var FORBIDDEN_CLASS_RE = /(?:^|\s)(?:site-header|site-footer|site-logo|nav-desktop|menu-hint|bg-fixed|bg-overlay|radial-menu|menu-fab|header-auth|cms-|radial-)(?:\s|$)/i;

  function isForbiddenEl(el) {
    if (!el || el.nodeType !== 1) return true;
    if (FORBIDDEN_TAGS[el.tagName]) return true;
    if (el.id === 'cms-ui' || el.id === 'cms-page-style') return true;
    if (el.className && FORBIDDEN_CLASS_RE.test(String(el.className))) return true;
    return false;
  }

  function isExplicitlyEditable(el) {
    if (!el || !el.getAttribute) return false;
    var v = el.getAttribute('data-editable') || el.getAttribute('data-cms-editable');
    return v === 'true' || v === '1' || v === '';
  }

  function isAllowedTextEl(el) {
    if (!el || el.nodeType !== 1) return false;
    if (ALLOWED_TEXT_TAGS[el.tagName]) return true;
    if ((!el.children || el.children.length === 0) && (el.textContent || '').trim().length > 0) {
      if (!STRUCTURAL_TAGS[el.tagName] && !FORBIDDEN_TAGS[el.tagName]) return true;
    }
    return false;
  }
  ...
  function pickTarget(el) {
    ...
    if (isForbiddenEl(el)) return null;

    var cur = el;
    var best = null;

    while (cur && cur !== doc().body && cur !== doc().documentElement) {
      if (isLocked(cur) || isForbiddenEl(cur)) return null;
      if (isExplicitlyEditable(cur)) { best = cur; break; }
      if (isAllowedTextEl(cur)) { best = cur; break; }
      cur = cur.parentElement;
    }

    if (!best) return null;
    if (STRUCTURAL_TAGS[best.tagName] && !isExplicitlyEditable(best)) return null;
    return best;
  }
  ...
  function buildPageCss() {
    var parts = ['/* Auto-generated by Reitansai CMS */', ''];
  ...
  function enterEdit(el) {
    var target = pickTarget(el);
    if (!target) {
      if (el && isLocked(el)) {
        status('data-lock="true" のため編集できません');
        try { showLockBadge(el.closest ? (el.closest('[data-lock="true"]') || el) : el); } catch (e) {}
      } else {
        status('この要素は編集対象外です（本文テキストまたは data-editable のみ）');
      }
      return;
    }
    el = target;
    if (el === doc().body || el === doc().documentElement) return;
    if (isChromeUi(el) || isForbiddenEl(el)) return;
    if (isLocked(el)) {
      status('data-lock="true" のため編集できません');
      clearSelection();
      return;
